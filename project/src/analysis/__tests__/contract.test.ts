...
function runTests() {
  ...
  try {
    const contracts = buildContracts(path.join(TEST_DIR, 'state.tsx'));
  ...
  console.log('\nTest 5: Multiple state variables');
  try {
    const contracts = buildContracts(path.join(TEST_DIR, 'multi-state.tsx'));
  ...
  console.log('\nTest 6: Python fallback analyzer returns contracts');
  try {
    const contracts = buildContracts(path.join(TEST_DIR, 'state.py'));
  ...
  console.log('\nTest 7: C++ fallback analyzer returns contracts');
  try {
    const contracts = buildContracts(path.join(TEST_DIR, 'state.cpp'));
