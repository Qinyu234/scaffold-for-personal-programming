...
class GraphPanel {
    ...
    static show(context, spec, onMessage) {
        ...
        const title = spec.tier === 'aggregation'
            ? `Lucid cluster: ${spec.scopeId}`
            : `Lucid ${spec.viewType}: ${spec.scopeId}`;
        const panel = vscode.window.createWebviewPanel('lucidGraph', title, vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true });
