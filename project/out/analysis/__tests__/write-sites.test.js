...
function runTests() {
    ...
    try {
        const writeSites = (0, write_sites_1.findWriteSites)(path.join(TEST_DIR, 'assignments.ts'));
    ...
    console.log('\nTest 3: Maps each assignment to the enclosing function or module');
    try {
        const writeSites = (0, write_sites_1.findWriteSites)(path.join(TEST_DIR, 'function-assignments.ts'));
    ...
    console.log('\nTest 5: Finds setState calls');
    try {
        const writeSites = (0, write_sites_1.findWriteSites)(path.join(TEST_DIR, 'setstate.tsx'));
...
//# sourceMappingURL=write-sites.test.js.map